...
"""
detectArbitrage
The input of the function is the currencies and tol, the tol is equal to 1e-15
The output of the function is a single list of vertex ranks corresponding to the arbitrage

"""
def detectArbitrage(currencies, tol=1e-15):
    for vertex in currencies.adjList:
        #Set the distance of the vertices to infinity
        vertex.dist=math.inf
        #Set the previous vertex to be None
        vertex.prev=None
        
    #The start distance should be 0
    currencies.adjList[0].dist=0
    for it in range(0,len(currencies.adjList)):
        s=-1
        for u in currencies.adjList:
            for neighbor in u.neigh:
                #If the old distance is bigger than the new distance plus tol
    ...
    #If s has not been updated in the last iteration, there is no cycle
    if(s==-1):
       return []
   
    #Else, we should find the cycle
    else:
        #Create an empty path
        path=[]
        neighbor = currencies.adjList[s]
        #See if a vertex has been revisited
        while len(path)==len(set(path)):              
            #If a vertex is revisited, put that into the path
            path.append(neighbor.rank)      
            #Trace back to put all the vertexes into the path
            neighbor=neighbor.prev
            
        #Find the index of the vertex which has been revisited
        for index in range(0,len(path)):
            if path[index]==path[len(path)-1]:
                break
            
        #Delete all the vertexes which are not in the cycle
        path=path[index:]
        #Reverse the path
        path.reverse()  
        return path
...
"""
rates2mat
The input of the function is the matrix of the rates
The output of the function is the log of the matrix of the rates
"""
def rates2mat(rates):
    # Currently this only returns a copy of the rates matrix.
    #Make the elements of the matrix to be log
