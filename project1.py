...
Partner 1: Zhenyu Yang             NetID:zy88
Partner 2:
Date: 11/02/2018
"""
...
def SelectionSort(A):
    length = len(A)
    if length==0 or length==1:    #if the length of the array is 0 or 1, it is already sorted
        return A
    for i in range(length):
        smallest=i                #A[smallest] is the smallest element of the rest of the array
        for j in range(i,length):
            if A[j]<A[smallest]:
                smallest=j
        if i!=smallest:           #Swap the first number of the rest of the array with the smallest number of that array
            temp = A[smallest]
            A[smallest] = A[i]
            A[i] = temp
    return A
...
def InsertionSort(A):
    ...
    for i in range(1,length):
        j=i
        target=A[i]
        while j>0 and target<A[j-1]: #find a first time where A[j]<A[i]
            A[j]=A[j-1]
            j=j-1
        A[j]=target
    return A
...
def BubbleSort(A):
    ...
    if length==0 or length==1:   #if the length of the array is 0 or 1, it is already sorted
        return A
    flag=1                       #flag is used to determine whether the array is sorted
    while flag==1 :              
        flag=0
...
def merge(A,left,mid,right):
    temp=[] #temp is a place to put the sorted array
    i=left
    j=mid+1
    while i<=mid and j<=right:
        if A[i]<=A[j]: #compare the elements of the two array
            temp.append(A[i])
            i=i+1
        else:
            temp.append(A[j])
            j=j+1
    while i<=mid:  #the size of left array and right array may not be the same
        temp.append(A[i])
        i=i+1
    while j<=right:
        temp.append(A[j])
...
def Msort(A,left,right): 
    if left>=right:
        return
    mid=(left+right)//2
    Msort(A,left,mid)           #recursive
    Msort(A,mid+1,right)        #recursive
...
"""
def pivot(A,i,j):
    key=i #the pivot is the first number of the array
    while i<j-1:
        while A[j-1]>A[key]:       #find j which is out of place
            j=j-1
        while A[i]<A[key]:         #find i which is out of place
            i=i+1
        if i== key:                #find key which is out of place and swap A[j-1] and A[key]
           (A[j-1],A[key])=(A[key],A[j-1])
           key=j-1
           i=i+1
        elif j-1 == key:           #find key which is out of place and swap A[i] and A[key]
            (A[i],A[key])=(A[key],A[i])
            key=i
            j=j-1
        else:                      #find A[i] and A[j-1] which are out of place and swap them
            (A[i],A[j-1])=(A[j-1],A[i])
            i=i+1
            j=j-1
    return key
...
def QuickSort(A, i, j):
    ...
    if length==0 or length==1:       #if the length of the array is 0 or 1, it is already sorted
        return A
    mid=pivot(A,i,j)
    QuickSort(A,i,mid)              #recursive
    QuickSort(A,mid+1,j)            #recursive
    return A
