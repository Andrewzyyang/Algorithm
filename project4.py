...
"""
"""
The inputs of the function are the adjacency list and the adjacency matrix, the output of the function is the minimum spanning tree
created by the Prim's Algorithm.
"""
def prim(adjList, adjMat): 
    ...
    #Pick up the adjList[0]to be the start vertex
    start=adjList[0]
    start.cost=0
    Q=MinQueue(adjList)
...
"""
"""
The inputs of the function is the adjacency lists and edgelists, the output of the function is the minimum spanning tree
created by Kruskal's Algorithm. 
"""
...
def kruskal(adjList, edgeList):
    ...
    for e in edgeList:
        #If the min edge crosses a cut, add it to the MST
        u,v = e.vertices
        #If the root of the set with vertex u is not equal to the root of the set with vertex v
        if not find(u).isEqual(find(v)):
            #Put e into the MST
            S.append(e)
            #Union the set with vertex u and the set with vertex e
            union(u,v)
    return S
...
"""
TSP
The input of this function is the adjList and start, the output of the function is the tour of this problem.
"""
def tsp(adjList, start):
    ...
    path=[]
    #Initialize the list tour
    tour = []
    #including the first node into the path
    path.append(start)
    #Go through the path
    while path:
        #Pop an element out of the path to be the current vertex
        current = path.pop()
        #Put the element into the tour list
        tour.append(current.rank)
        #set the current node to be already visited
        current.visited=True
        for neighbor in current.mstN:
            #If neighbor has not been visited, put it into the path
            if neighbor.visited==False:
                path.append(neighbor)
    #Put the start node of the tour into the tour list.
    tour.append(start.rank)
    #Reverse the tour list
    tour.reverse()
    return tour
...
"""
"""
The input of the function is the vertex, the output of the function is the singleton set created by the function.
"""
def makeset(v):
    v.pi=v
    #Set the height of the vertex to be 0
    v.height=0
    return
...
"""
"""
The input of the function is the vertex, the output of the function is the root of the set that contains the vertex
"""
...
"""
The inputs of the function are vertices u and vertices v, the output of the function is the result of the union sets of vertices v and iu. 
"""
def union(u,v):
    ...
    rv=find(v)
    #The sets ru and rv are the same
    if ru.isEqual(rv):
        return
    #Make shorter set point to the taller set
    if ru.height>rv.height:
        rv.pi=ru
    elif ru.height<rv.height:
        ru.pi=rv
    else:
        #If the height of the set with vertex u is the same as the height of the set with vertex v, break tie
        ru.pi=rv
